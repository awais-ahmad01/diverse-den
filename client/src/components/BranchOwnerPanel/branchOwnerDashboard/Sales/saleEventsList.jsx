...
import { listSaleEvents, getSaleEventById, deleteSaleEvent } from "../../../../store/actions/saleEvents";
import {
  Table,
  TableBody,
  TableCell,
...
const EventDetailsDialog = ({ open, onClose, event }) => {
  ...
  useEffect(() => {
    if (event && open) {
      ...
      dispatch(getSaleEventById(eventId));
    }
  }, [event, open, dispatch]);
  ...
  return (
    <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
      <DialogTitle className="bg-[#603F26] text-white">
        Sale Event Details - {saleEventById?.name}
      </DialogTitle>
      <DialogContent className="mt-4">
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={6}>
            ...
            <p>
              Type:{" "}
              {saleEventById?.discountType}
            </p>
            <p>
              Value:{" "}
              {saleEventById?.discountValue}
            </p>
            <p>
              Duration: {saleEventById?.duration}
            </p>
          </Grid>
          ...
          <Grid item xs={12}>
            ...
            <h3 className="text-base font-bold mb-2">
              Products Included ({saleEventById?.productsIncluded})
            </h3>
            <TableContainer>
              <Table>
                <TableHead>
...
const SaleEventsList = () => {
  ...
  const { saleEvents, meta, isloading } = useSelector((state) => state.saleEvents);
